...
const sourceFiles = [
  ...
  "apps/desktop/src/pointerEdgeModel.ts",
  "apps/desktop/src/pointerEdgeModel.test.ts"
].map((file) => path.join(repoRoot, file));

const outDir = await mkdtemp(path.join(tmpdir(), "onstell-pointer-edge-tests-"));
...
try {
  ...
  const testModule = require(path.join(outDir, "pointerEdgeModel.test.js"));
  const count = testModule.runPointerEdgeModelTests();
  console.log(`pointer edge model tests passed (${count})`);
} finally {
