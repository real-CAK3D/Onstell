...
const sourceFiles = [
  ...
  "apps/desktop/src/routingModel.test.ts"
].map((file) => path.join(repoRoot, file));

const outDir = await mkdtemp(path.join(tmpdir(), "onstell-routing-tests-"));
...
try {
  ...
  const testModule = require(path.join(outDir, "routingModel.test.js"));
  const count = testModule.runRoutingModelTests();
  console.log(`routing model tests passed (${count})`);
} finally {
